...
// async function for fetching and loading statelist from a json file
async function loadStates() {

    const response = await fetch("data/states.json");
    const states = await response.json();

    const select = document.getElementById("stateList");
    ...
    for (const abbreviation in states) {
        ...
        option.textContent = abbreviation;

        select.appendChild(option);
    }

    // run the loadstored values function after the states are loaded
    loadStoredValues();
...
async function loadReflection() {
    ...
    const response = await fetch("data/reflection.json");
    const paragraphs = await response.json();

    const reflectionDiv = document.getElementById("notes");

    // default empty option
    reflectionDiv.innerHTML = '<h1>Synopsis of HW4</h1>';
...
function validateField() {
    ...
    if ((this.id === 'stateList') || (this.id === 'zipcode')) {
        ...
        if (value === '' || !zipPattern.test(document.getElementById('zipcode').value)) {
...
// buttons regarding forget me cookie and local storage functions
function forgetMeFunc() {
    deleteCookie("fname");
    deleteCookie("lname");
    deleteCookie("ssn");
    parent.checkCookie();
...
// resets style of inputfields
function resetStyle() {
    for( let i=0; i<inputFields.length; i++ ) 
    {
        inputFields[i].style.border = "";
    }
    document.getElementById("stateList").style.border = '';
    clearErrors(0);
}

// clears values when new user button is clicked
function clearValues() {
    //loop through input fields and set values to null
    for( let i=0; i<inputFields.length; i++)
    {
    ...
    //set text area value to null
    document.getElementById('perMessage').value = '';
}

// clears the all local storage
function clearLocalStorage() {
    localStorage.clear()
}

// saves fieldids and their associated value to the local storage
function saveToLocalStorage(event) {
    const field = event.target;
    localStorage.setItem(field.id, field.value);
}

// loads locally stored values and assigns them to the input fields
function loadStoredValues() {

    const fname = document.getElementById("fname");
    const mname = document.getElementById("minitial");
    ...
    const vacinated = document.querySelector('input[name="vac"]:checked');
    const insurance = document.querySelector('input[name="insurance"]:checked');
    const message = document.getElementById("perMessage");
    let storedDiseases = JSON.parse(localStorage.getItem("disease")) || [];
    ...
    storedDiseases.forEach(function(value) {
        let box = document.querySelector(`input[name="disease"][value="${value}"]`);
    ...
    document.getElementById("minitial").value = localStorage.getItem("minitial") || "";
    ...
    document.getElementById("perMessage").value = localStorage.getItem("perMessage") || "";

    document.getElementById("feel").value = localStorage.getItem("feel") || "5";
    ...
    
{
    ...
    //sets cookies and local storage for non-sensistive fields skipping and hiding sensitive info
    if (inputFields[i].id === 'ssn' ||
        inputFields[i].id === 'dateOfBirth' ||
        inputFields[i].id === 'addressLineField1' ||
        inputFields[i].id === 'usr' ||
        inputFields[i].id === 'password' ||
        inputFields[i].id === 'passwordConfirm')
        {
        ...
        {
            inputFields[i].addEventListener("blur", function () {
                setCookie(this.id, this.value.trim(), 2);
                parent.checkCookie();
            });

            inputFields[i].addEventListener("input", saveToLocalStorage);
        }

    
    //sets realtime event listernes for input fields
...
//event listener for save to local storage function call
document.getElementById('perMessage').addEventListener("input", saveToLocalStorage);

document.getElementById("stateList").addEventListener("change", saveToLocalStorage);
...
// event listener to reset form style
form.addEventListener("reset", resetStyle);

// reset cookies and local storage
const newUser = document.getElementById("newUser");
newUser.addEventListener("click", forgetMeFunc);

// event listener to review form
document.getElementById("review").addEventListener("click", reviewInfo);

//load states list into select html
window.addEventListener('load', loadStates);

//loads reflection html and data
window.addEventListener('load', loadReflection);
